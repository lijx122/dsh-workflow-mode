...
dsh-workflow-mode/
├─ packages/
│  ├─ dsh-workflow-schema/        # DSL Schema 与校验器 (T2)
│  ├─ dsh-workflow-engine/        # DAG 引擎核心 (T3/T4/T5)
│  └─ dsh-client-ui-workflow/     # Web GUI React Flow 画布 (T8)
├─ config/agent-presets/workflow/ # Preset 注册 (T7)
└─ examples/workflows/            # 示例工作流 JSON (T9)
```
